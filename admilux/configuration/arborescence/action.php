...
{
	if (!(file_exists($_GET['fichier']))) message3("L'image $_GET[fichier] n'existe pas . Sélectionnez un fichier valide ");
	else 
	{
...
if (  isset($_GET['action']) and $_GET['action']=="ajouter_fichier"  )
	{
	$content_dir = $_GET['dossier'];// dossier où sera déplacé l'image
	...
	$name_file=$content_dir ."/". $name_file;
		...
		else message3("L'image $name_file1 a été téléchargé dans le dossier $dossier_affichage");
	}
...
message3("- Pour visualiser, supprimer ou renommer une image, s&eacute;lectionnez l'image concern&eacute;e.<br>
  ...
  dans lequel vous voulez t&eacute;l&eacute;charger l'image. (extension acceptée : .gif,.jpg et .png)<br>
- Pour voir les modifications, n'oubliez pas d'actualiser l'arborescence du dossier -images-
<br>- Ne supprimez pas les images du dossier -config- . Vous pouvez les modifier sans problème, les attributs de taille ont été supprimés des balises.
 ");
?>